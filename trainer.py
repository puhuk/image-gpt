...
class Trainer:
    ...
    def train(self):
        ...
        model = torch.nn.DataParallel(model, self.config.devices)
        ...
        def run_epoch(split):
            ...
            for it, (x, y) in pbar:
                ...
                # print("DEBUJG", x.shape)

                x = x.view(x.shape[0], -1)
                x = x.transpose(0, 1).contiguous()

                # print("DEBUJG2", x.shape)
                ...
                if is_train:
                    ...
                    # report progress
                    pbar.set_description(f"epoch {epoch+1} iter {it}: train loss {loss.item():.5f}. lr {lr:e}. shape {logits.shape}")
